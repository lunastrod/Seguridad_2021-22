...
void send_request(int connfd, int64_t nonce, struct account acc){
    ...
    send(connfd, &msg.r, sizeof(msg.r), 0);
    send(connfd, &msg.t, sizeof(msg.t), 0);
    send(connfd, &msg.login, sizeof(msg.login), 0);
}
...
int recv_request(int connfd, int64_t nonce, FILE * accounts, char * login_name){
    struct auth_request_msg msg;
    recv(connfd, &msg.r, sizeof(msg.r),0);
    recv(connfd, &msg.t, sizeof(msg.t),0);
    recv(connfd, &msg.login, sizeof(msg.login),0);
